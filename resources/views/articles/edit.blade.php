...
@section('content')
    ...
    <h3><i class="fa fa-plus"></i> Update article</h3>
    @if ($errors->any())
    ...
    <form method="POST" action="{{ route('articles.update', $article->id)}}" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col">
                <div id="form-en">
                    <div>
                        <input type="text" name="title_en" id="title_en" placeholder="Title (EN)" value="{{ $article->title_en }}" required>
                    </div>
                    <div>
                        <textarea name="description_en" placeholder="Short Description (EN)" rows="2">{{ $article->description_en }}</textarea>
                    </div>
                    <div>
                        <textarea name="body_en" class="bodies" id="body_en" placeholder="Content (EN)" rows="6">{{ $article->body_en}}</textarea>
                    </div>
                </div>
                <hr>
                <div id="lang-options">
                    <div class="row">
                        <div class="col-auto">
                            <input type="checkbox" name="ku_check" @if ($article->title_ku)
                            checked
                            @endif 
                            id="ku_check">
                        ...
                        <div class="col-auto">
                            <input type="checkbox" name="ar_check" @if ($article->title_ar)
                                checked
                                @endif id="ar_check">
                ...
                <div id="form-ku" style="display:none;">
                    <div>
                        <input type="text" name="title_ku" value="{{ $article->title_ku }}" placeholder="Title (KU)">
                    </div>
                    <div>
                        <textarea name="description_ku" placeholder="Short Description (KU)" rows="2">{{ $article->description_ku }}</textarea>
                    </div>
                    <div>
                        <textarea name="body_ku" class="bodies" placeholder="Content (KU)" rows="6">{{ $article->body_ku}}</textarea>
                    </div>
                </div>
                <div id="form-ar" style="display:none;">
                    <div>
                        <input type="text" name="title_ar" value="{{ $article->title_ar }}" placeholder="Title (آر)">
                    </div>
                    <div>
                        <textarea name="description_ar" placeholder="Short Description (آر)" rows="2">{{ $article->description_ar }}</textarea>
                    </div>
                    <div>
                        <textarea name="body_ar" class="bodies" placeholder="Content (آر)" rows="6">{{ $article->body_ar }}</textarea>
                    </div>
                </div>
            </div>
            <div class="col-1"></div>
            <div class="col">
                ...
                <div class="row">
                    <div class="col-auto">
                        ...
                        <input type="checkbox" name="featured" {{$article->featured ==1? 'checked':''}} id="featured">
                    ...
                    <select name="tags[]" id="tags" data-placeholder="Select Tags" class="select" multiple>
                        @if ($tags)
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" 
                                {{  
                                    in_array($tag->id, $article_tags) ? 'selected' : '' 
                                }}
                                >{{ $tag->name_en }}
                                </option>
                            @endforeach
                        ...
                        @if ($sponsers)
                            @foreach ($sponsers as $sponser)
                                <option value="{{ $sponser->id }}"
                                    {{  
                                        in_array($sponser->id, $article_sponsers) ? 'selected' : '' 
                                    }}
                                    >{{ $sponser->name }}
                                </option>
                            @endforeach
                        ...
                        @if ($profiles)
                            @foreach ($profiles as $profile)
                                <option value="{{ $profile->id }}" 
                                    ...
                                    {{
                                        in_array($profile->id, $article_profiles) ? 'selected' : '' 
                                    }}
